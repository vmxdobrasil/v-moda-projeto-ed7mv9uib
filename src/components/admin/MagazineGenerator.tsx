...
export function MagazineGenerator() {
  ...
  const download = async () => {
    ...
    try {
      ...
      try {
        img = await loadImage(prod.image)
      } catch {
        toast({
          title: 'Aviso',
          description: 'Imagem não pôde ser carregada com CORS, gerando sem imagem.',
        })
      }

      const hh = Math.floor(h * 0.1)
      const fh = Math.floor(h * 0.2)
      const fy = h - fh
      ctx.fillStyle = '#000'
      ...
      if (img) {
        ...
        try {
          ...
          // Canvas tainted — skip image
        }
      } else {
        ctx.fillStyle = '#f3f4f6'
        ctx.fillRect(0, hh, w, ih)
      }
      ...
      try {
        ...
        toast({
          title: 'Aviso',
          description: 'Exportação parcial devido a restrições CORS. Tente outra imagem.',
        })
      }
    } catch {
  ...
  const handlePublish = () => {
    ...
    toast({ title: 'Publicado!', description: 'Layout adicionado à Revista Digital.' })
  }
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex gap-4">
            <Button className="flex-1" size="lg" disabled={!prod || generating} onClick={download}>
              {generating ? (
